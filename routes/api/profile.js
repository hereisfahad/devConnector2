...
router.get("/user/:user_id", async (req, res) => {
  try {
    ...
    if (profile?.handle) {
      ...
      return res.json({
        profile,
        repos: data.map(({ clone_url, name, description, forks, watchers }) => ({
          clone_url,
          name,
          description,
          forks,
...
router.get("/github/:username", async (req, res) => {
  ...
  try {
    const { data } = await axios.get(
      `https://api.github.com/users/${username}/repos?per_page=${limit}&sort=created:asc&client_id=${process.env.githubClientId}`
    );
    res.json({ repos: data });
  } catch (error) {
    res.status(500).send("Server Error");
